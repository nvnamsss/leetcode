...
class Solution {
    void recursive(vector<string>& rs, map<string, vector<string>>& graph, set<string>& visited, string node) {
        visited.insert(node);
        rs.push_back(node);
        ...
        for (int i = 0; i < paths.size(); i++)
        {
            if (!visited.count(paths[i])) {
                recursive(rs, graph, visited, paths[i]);
            }
        }
    ...
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        for (int i = 0; i < accounts.size(); i++)
        {
            if (!visited.count(accounts[i][1])) {
                ...
                recursive(subset, graph, visited, accounts[i][1]);
